...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    # print len(data), N, num_landmarks
    Ndim=2*(1+num_landmarks)
    Omega=matrix()
    Omega.zero(Ndim, Ndim)
    Xi=matrix()
    Xi.zero(Ndim,1)
    Omega.value[0][0]=1.0
    Omega.value[1][1]=1.0
    ...
    for i in range(len(data)):
        ...
        for j in range(len(measures)):
            m=measures[j][0]
            mm=m*2+2
            dmeasure=measures[j][1:]
            for b in range(2):
                Omega.value[0+b][0+b] += 1/measurement_noise
                Omega.value[0+b][mm+b] += -1/measurement_noise
                Omega.value[mm+b][0+b] += -1/measurement_noise
                Omega.value[mm+b][mm+b] += 1/measurement_noise
                Xi.value[0+b][0] += -dmeasure[b]/measurement_noise
                Xi.value[mm+b][0] += dmeasure[b]/measurement_noise

        ExList=[0,1]+list(range(4,Ndim+2))
        Omega=Omega.expand(Ndim+2, Ndim+2, ExList, ExList)
        Xi=Xi.expand(Ndim+2, 1, ExList, [0])

        dposition=data[i][1]
        for b in range(2):
            Omega.value[0+b][0+b] += 1/motion_noise
            Omega.value[0+b][2+b] += -1/motion_noise
            Omega.value[2+b][0+b] += -1/motion_noise
            Omega.value[2+b][2+b] += 1/motion_noise
            Xi.value[0+b][0] += -dposition[b]/motion_noise
            Xi.value[2+b][0] += dposition[b]/motion_noise

        Tlist=list(range(2,Ndim+2))
        Omega_p=Omega.take(Tlist, Tlist)
        Xi_p=Xi.take(Tlist, [0])
        a=Omega.take([0,1],Tlist)
